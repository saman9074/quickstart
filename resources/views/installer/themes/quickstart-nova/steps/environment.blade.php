...
@section('content')
    ...
    <form method="POST" action="{{ route('quickstart.install.environment') }}" class="space-y-6">
        ...
        @else
            @php
                ...
                foreach ($groups as $groupName => $keysInGroup) {
                    foreach ($keysInGroup as $key) {
                        if (isset($envFields[$key])) { // Only consider keys that are actually defined in $envFields
                ...
                // Determine the path for the env_field partial with fallback
            ...
            <fieldset class="border border-gray-300 dark:border-gray-700 p-4 sm:p-5 rounded-lg shadow-sm bg-white dark:bg-slate-800/50">
                <legend class="text-md sm:text-lg font-semibold text-gray-800 dark:text-gray-200 px-2 flex items-center">
